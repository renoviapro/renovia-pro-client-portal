...
export default function Tickets() {
  ...
  return (
    <div>
      ...
      <Link to="/tickets/new" className="mt-6 inline-block rounded-full bg-[#febd17] px-8 py-3 font-semibold text-black no-underline">
        Créer un ticket SAV
      </Link>
      <div className="mt-8 space-y-4">
        {items.length === 0 && <p className="text-white/50">Aucun ticket.</p>}
        {items.map((t) => (
          <Link key={t.id} to={`/tickets/${t.id}`} className="block rounded-xl border border-white/20 bg-white/5 p-4 no-underline">
